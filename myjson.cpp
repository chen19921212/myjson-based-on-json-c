...
Jbase::~Jbase() {}
string Jbase::toJsonString() const
{
...
Jbase* Jbase::fromJsonString(const string _jsonStr)
{
...
template <typename Type> J<Type>::J() : _val(Type()) {}
template <typename Type> J<Type>::J(const Type _v) : _val(_v) {}
template <typename Type> Type J<Type>::get() const { return _val; }
...
Jtype J<array>::getType() const { return JSON_ARRAY; }
int J<array>::length() const { return _jarr.size(); }
void J<array>::add(Jbase* _val) { _jarr.push_back(_val); }
void J<array>::put(const int _idx, Jbase* _val)
{
    if (_jarr.size() > _idx)
    {
        if (_jarr[_idx] != NULL)
...
Jbase* J<array>::get(const int _idx)
{
    if (_idx >= _jarr.size()) return NULL;
    return _jarr[_idx];
}
const Jbase* J<array>::get(const int _idx) const
{
    if (_idx >= _jarr.size()) return NULL;
    return _jarr[_idx];
}
...
Jtype J<object>::getType() const { return JSON_OBJECT; }
void J<object>::add(const string _key, Jbase* _val)
{
    del(_key);
    _jkeys.insert(_key);
    _jmap[_key] = _val;
}
void J<object>::del(const string _key)
{
...
Jbase* J<object>::get(const string _key) { return _jmap[_key]; }
const Jbase* J<object>::get(const string _key) const
{
    map<string, Jbase*>::const_iterator iter = _jmap.find(_key);
    if (iter != _jmap.end()) return iter->second;
    return NULL;
}
const set<string>& J<object>::getKeyArray() const { return _jkeys; }
